...
let OrdersByProductController = {
  GetAll: async (req, res) => {
    var ordersByProduct = await models.OrderByProduct.find({});
    res.json(ordersByProduct)
  },
  Get: async (req, res) => {
    /*  Validar ObjectId incorrectos que lleguen por param. */
    var orderByProduct = await models.OrderByProduct.findById(req.params.id);
    if (orderByProduct) {
        res.status(200).json(orderByProduct);
    } else {
        res.sendStatus(404);
    }
  },
...
module.exports = OrdersByProductController;
